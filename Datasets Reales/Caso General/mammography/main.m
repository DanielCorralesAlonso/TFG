...
%cvx_setup
%cvx_solver mosek
addpath('C:\Users\Almacen2\Documents\dani\Universidad\TFG\Código\Datasets Reales\Caso General\Funciones')

Dataset = readmatrix('mammographic.csv');
n = length(Dataset(:,1));
...
psi_X = rnd_fou_feat(normalised_instances,300, d);

labels = Dataset(:,d+1) + 1;
...
for j = 1:length(alpha_list)
    ...
    for k = 1:10
        ...
        elseif alpha == Inf
            [opt_val, opt_mu, opt_nu] = fit_01loss(X_train,Y_train);
            [h_vect] = pred_01loss(opt_mu, opt_nu, X_test, T);
        else
            [opt_val, opt_mu, opt_nu] = fit_alphaloss(X_train, Y_train, alpha);
            [h_vect] = pred_alphaloss(opt_mu,opt_nu, alpha, X_test, T);
        end
